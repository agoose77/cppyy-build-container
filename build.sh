...
popd
auditwheel repair /tmp/wheels/cppyy-cling* --plat "$PLAT" -w /wheelhouse
    
# Install cppyy-cling, build cppyy-backend without isolation and don't check versions
python3 -m pip install /wheelhouse/cppyy-cling*
pushd ./cppyy-backend/clingwrapper 
python3 -m build -n -x -w . -o /tmp/wheels/
popd
auditwheel repair /tmp/wheels/cppyy_backend* --plat "$PLAT" -w /wheelhouse
...
python3 -m build -n -x -w . -o /tmp/wheels/
popd
auditwheel repair /tmp/wheels/CPyCppyy* --plat "$PLAT" -w /wheelhouse

# Install CPyCppyy, build cppyy, install cppyy (pure Python)
python3 -m pip install /wheelhouse/CPyCppyy*
pushd ./cppyy 
